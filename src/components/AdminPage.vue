<template>
    <div>
        ...
        <button @click="clearResults">Barcha natijalarni o‘chirish</button>
        ...
        <ul>
            <li v-for="(question, index) in questions" :key="index">
                <div v-if="editedIndex === index">
                    <input v-model="editedQuestion.text" placeholder="Savolni kiriting" />
                    <ul>
                        <li v-for="(option, i) in editedQuestion.options" :key="i">
                            <input v-model="editedQuestion.options[i]" :placeholder="'Variant ' + (i + 1)" />
                        </li>
                    </ul>
                    <input v-model="editedQuestion.correct" placeholder="To‘g‘ri javobni kiriting" />
                    <button @click="saveEdit(index)">Saqlash</button>
                    <button @click="cancelEdit">Bekor qilish</button>
                </div>
                <div v-else>
                    {{ question.text }}
                    <ul>
                    ...
                    <button @click="editQuestion(index)">Tahrirlash</button>
                </div>
            </li>
        </ul>
...
export default {
    data() {
        return {
            ...
            editedIndex: null,
            editedQuestion: { text: "", options: ["", "", "", ""], correct: "" }
        };
    },
    methods: {
        clearResults() {
        ...
        editQuestion(index) {
            ...
            this.editedQuestion = JSON.parse(JSON.stringify(this.questions[index]));
        },
        saveEdit(index) {
            this.questions[index] = JSON.parse(JSON.stringify(this.editedQuestion));
